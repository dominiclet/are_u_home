use core::time;
use std::{env, thread::{sleep, self}, collections::HashMap, sync::{Arc, Mutex}, time::SystemTime, alloc::System};

use constants::HELP_COMMAND;
use telegram_apis::TelegramClient;
use types::{GroupInfo, Update, Message};

use crate::constants::UPDATE_INTERVAL;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        for update in updates {
            ...
            match command {
                types::Command::Help => help_handler(&update,&telegram_client),
                types::Command::Start => start_handler(&update, &going_home_groups, &telegram_client),
                types::Command::Homed => homed_handler(&update, &going_home_groups, &telegram_client)
            };
        }
...
fn homed_handler(update: &Update, going_home_groups: &Arc<Mutex<HashMap<i64, GroupInfo>>>, telegram_client: &TelegramClient) {
    ...
    println!("Attempting to retrieve lock");
